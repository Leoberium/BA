...
def decoder(s, transition, emission):
    nc = int(list(transition.keys())[-2][1:])

    # creating the graph
    graph = [{'S': [0, '']}]
    graph[0].update({key: [0, ''] for key in transition if key[0] == 'D'})
    for i in range(len(s)):
        graph.append({key: [0, ''] for key in transition if key[0] not in ('S', 'E')})
    graph.append({'E': [0, '']})

    # pre-computing the first column
    prev = 'S'
    for node in graph[0]:
        if node == 'S':
            continue
        graph[0][node] = [log(transition[prev][node]) + graph[0][prev][0], prev]
        prev = node

    # computing the whole graph except sink
    for j in range(1, len(s) + 1):
        for node in graph[j]:
            col = j if node[0] == 'D' else j - 1

            max_weight, max_state = -10**9, ''
            for state in in_edges(node, nc):
                if state not in graph[col]:
                    continue
                score = graph[col][state][0]
                t = transition[state][node]
                e = emission[node][s[j-1]] if node[0] != 'D' else 1
                weight = score + log(t * e)
                if weight > max_weight:
                    max_weight = weight
                    max_state = state
            graph[j][node] = [max_weight, max_state]

    # computing the sink
    max_weight, max_state = -10**9, ''
    for state in in_edges('E', nc):
        score = graph[-2][state][0]
        t = transition[state]['E']
        weight = score + log(t)
        if weight > max_weight:
            max_weight = weight
            max_state = state
    graph[-1]['E'] = [max_weight, max_state]
...
def main():
    ...
    path = decoder(text, transition, emission)
    print(*path)
