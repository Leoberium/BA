...
def longest_shared_substring(s1, s2):
    ...
    while -1 in color.values():
        ...
        for node in filter(lambda x: color[x] == -1, color.keys()):
            ...
            break
    # searching for the shallowest internal node belonging to the first string
    min_depth, sh_node = max(depth.values()), root
    for node in filter(lambda x: color[x] == 0, color.keys()):
        if node.is_leaf():
            continue
    ...
    r = ''
    while sh_node != root:
        r = s[sh_node.start:sh_node.end] + r
        sh_node = sh_node.parent
    return r
