...
def alignment_with_affine_gap_penalties(s1, s2, score, s, e):
    n, m = len(s1), len(s2)
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            lower = max(d[i-1][j][0] - e, d[i-1][j][2] - s)
            upper = max(d[i][j-1][1] - e, d[i][j-1][2] - s)
            middle = max(lower, upper, d[i-1][j-1][2] + score[s1[i-1]][s2[j-1]])
            d[i][j] = (lower, upper, middle)
    # backtrack
    i, j = n, m
    r1, r2 = '', ''
    while i > 0 or j > 0:
        if i > 0 and d[i][j][2] == d[i][j][0]:
            r1 += s1[i-1]
            r2 += '-'
            i -= 1
            continue
        if j > 0 and d[i][j][2] == d[i][j][1]:
            r1 += '-'
            r2 += s2[j-1]
            j -= 1
            continue
        if i > 0 and j > 0 and d[i][j][2] == d[i-1][j-1][2] + score[s1[i-1]][s2[j-1]]:
            r1 += s1[i-1]
            r2 += s2[j-1]
            i -= 1
            j -= 1
    return d[n][m][2], r1[::-1], r2[::-1]


def main():
    v = sys.stdin.readline().strip()
    w = sys.stdin.readline().strip()
    for o in alignment_with_affine_gap_penalties(s1=v, s2=w, score=blosum62, s=11, e=1):
        print(o)
